...
ninja_file.variable('LDFLAGS', '-T chameleontwist.ld -T undefined_syms_auto.txt -T undefined_syms.txt -Map build/chameleonTwistUS.map --no-check-sections')
...
ninja_file.rule('make_z64',
    command = '($OBJCOPY -O binary $in $out) && (sha1sum -c chameleonTwistUS.sha1)',
...
ninja_file.build("build/chameleonTwistUS.elf", "make_elf ", o_files)
ninja_file.build("build/chameleonTwistUS.z64", "make_z64 ", "build/chameleonTwistUS.elf")
ninja_file.build("build/chameleonTwistUS.ok", "make_expected ", "build/chameleonTwistUS.z64")
